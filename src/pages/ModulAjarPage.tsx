...
interface ModulAjarPageProps {
  onBack: () => void;
}

const ModulAjarPage: React.FC<ModulAjarPageProps> = ({ onBack }) => {
  ...
  const handleGenerate = async () => {
    ...
    const prompt = `Buatkan MODUL AJAR KURIKULUM MERDEKA lengkap untuk Mapel: ${subject}, Topik: ${topic}, Fase: ${fase}.`;
    const instruction = "Anda adalah pakar kurikulum merdeka. Buat modul yang sistematis, rapi, dan baku.";
  ...
  return (
    <div className="animate-in slide-in-from-bottom-4 duration-500 p-8 max-w-6xl mx-auto">
      ...
      <div className="bg-white rounded-[48px] border border-slate-100 p-12 shadow-2xl shadow-slate-200/50 no-print">
        <div className="mb-12">
          <h2 className="text-4xl font-black text-slate-900 tracking-tight">Generator Modul Ajar</h2>
          <p className="text-slate-500 font-medium mt-2 text-lg">Susun administrasi Kurikulum Merdeka otomatis.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-10">
          <div>
            ...
            <input type="text" value={subject} onChange={(e) => setSubject(e.target.value)} placeholder="Contoh: Fisika" className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:border-blue-500 font-bold transition-all" />
          </div>
          <div>
            <label className="block text-[11px] font-black text-slate-400 uppercase tracking-widest mb-2 ml-1">Topik Utama</label>
            <input type="text" value={topic} onChange={(e) => setTopic(e.target.value)} placeholder="Contoh: Energi Terbarukan" className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:border-blue-500 font-bold transition-all" />
          </div>
          <div>
        ...
        <button onClick={handleGenerate} disabled={loading} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-black py-7 rounded-3xl shadow-xl transition-all disabled:opacity-50 text-xl uppercase tracking-[0.2em]">
          {loading ? 'SEDANG MENYUSUN...' : 'MULAI GENERATE OTOMATIS'}
        </button>
      </div>
      ...
      {result && (
        <div className="mt-16 animate-in fade-in duration-700">
          <div className="flex flex-wrap items-center justify-between gap-4 mb-8 no-print bg-white p-8 rounded-[32px] border border-slate-100 shadow-lg">
            <h3 className="text-2xl font-black text-slate-900 tracking-tight">Pratinjau Modul</h3>
