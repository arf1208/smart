...
const callAI = async (prompt: string, isJson: boolean = false) => {
  try {
    // Gunakan API_KEY (dari dialog pemilihan) atau GEMINI_API_KEY (default platform)
    const apiKey = process.env.API_KEY || process.env.GEMINI_API_KEY;
    
    if (!apiKey || apiKey === "undefined") {
      throw new Error("Gemini API Key tidak ditemukan. Silakan pilih API Key melalui tombol aktivasi.");
    }
    ...
    if (!text) {
      throw new Error("AI tidak memberikan respon (mungkin terblokir filter keamanan atau kuota habis).");
    }
