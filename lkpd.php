...
<main class="max-w-6xl mx-auto p-8">
    ...
    <div class="bg-white rounded-[48px] border border-slate-100 p-12 shadow-2xl shadow-slate-200/50 no-print">
        <div class="mb-12">
            <h2 class="text-4xl font-black text-slate-900 tracking-tight">Generator LKPD</h2>
            <p class="text-slate-500 font-medium mt-2 text-lg">Lembar Kerja Peserta Didik yang kontekstual.</p>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-10">
            <div>
                <label class="input-label">Mata Pelajaran</label>
                <input type="text" x-model="formData.subject" placeholder="Contoh: Kimia" class="serasi-input">
            </div>
            <div>
                <label class="input-label">Topik Utama</label>
                <input type="text" x-model="formData.topic" placeholder="Contoh: Laju Reaksi" class="serasi-input">
            </div>
            <div>
        ...
        <button @click="handleGenerate('lkpd')" :disabled="loading" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-black py-7 rounded-3xl shadow-xl transition-all disabled:opacity-50 text-xl uppercase tracking-[0.2em]">
            <span x-show="!loading">GENERATE LKPD</span>
    ...
    <div x-show="result" class="mt-16 animate-in fade-in duration-700">
        <div class="flex flex-wrap items-center justify-between gap-4 mb-8 no-print bg-white p-8 rounded-[32px] border border-slate-100 shadow-lg">
            <h3 class="text-2xl font-black text-slate-900 tracking-tight">Pratinjau LKPD</h3>
        ...
        <div class="bg-white p-16 md:p-24 rounded-[60px] shadow-2xl border border-slate-50 print:shadow-none print:p-0">
            <div class="prose prose-slate max-w-none leading-relaxed text-lg text-slate-800 whitespace-pre-line" x-html="formatResult(result)"></div>
        </div>
    </div>
</main>
